...
:set ($NotificationFunctions->"gotify") do={
  ...
  :local Headers ({ ("X-Gotify-Key: " . $Token);  "Content-Type: application/json" });

  :local Message;
  :if ([ :len [ :tostr ($Notification->"priority") ] ] > 0) do={
    :set Message ({"title"="[" . $IdentityExtra . $Identity . "] " . ($Notification->"subject");
  ...
  :do {
    ...
    :set ($Message->"title") ("[Queued] " . ($Notification->"subject"));
    :set ($Message->"message") (($Notification->"message") . "\n[This message was queued at " . \
      [ /system/clock/get date ] . " " . [ /system/clock/get time ] . "]");
    :set ($GotifyQueue->[ :len $GotifyQueue ]) { url=$Url; headers=$Headers; message=$Message };
...
:set SendGotify do={ 
  :do {
    ...
    $SendGotify2 ({ origin=$0; subject=$1; message=$2; priority=$3 });
  } on-error={
    :global ExitError; $ExitError false $0;
  }
}
